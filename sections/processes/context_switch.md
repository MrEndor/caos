...
```
   ...
   VOLUNTARY      INVOLUNTARY            INVOLUNTARY
       │             │                      │
       ▼             ▼                      ▼
 syscall      timer interrupt        wakeup higher-prio
 read() ждёт  (CONFIG_HZ Гц)         (RT thread прошёл
 данных                              wait_event_*)
 futex_wait   обновляет vruntime,
 nanosleep    помечает need_resched  irq → try_to_wake_up()
       │             │                      │
       └─────────────┼──────────────────────┘
                     ▼
        ┌──────────────────────────┐
        ...
        └────────────┬─────────────┘
                     ...
                     ▼
        ┌──────────────────────────┐
        │   pick_next_task()       │  опрашивает sched_class'ы
        └────────────┬─────────────┘    по приоритету
                     │
                     ▼
        ┌──────────────────────────┐
        │   context_switch()       │
        │     switch_mm()          │  load CR3 (если новый mm)
        │     switch_to()          │  asm: меняет стек, регистры
        │     finish_task_switch() │  cleanup, освобождение prev
        └────────────┬─────────────┘
                     │
                     ▼
       ┌─────────────────────────┐
...
schedule()                            kernel/sched/core.c
   │
   ▼
__schedule()                          выбирает next, проверяет preempt-disabled
   │
   ▼
pick_next_task()                      опрашивает sched_class'ы
   │
...
context_switch(rq, prev, next)
   ├── switch_mm_irqs_off(prev->mm, next->mm, next)   ← смена адресного пр-ва
   ├── switch_to(prev, next, prev)                    ← макрос, разворачивается в:
   │      __switch_to_asm(prev, next)                 arch/x86/entry/entry_64.S
   │         │ (asm: save callee-saved, swap stacks)
   │         ▼
   │      __switch_to(prev, next)                     arch/x86/kernel/process_64.c
   │         │ (C: FPU, TLS, debug regs, IO bitmap)
   │         ▼
   │      возврат — но уже в context next-task!
   └── finish_task_switch(prev)                       cleanup освобождённой prev
```
...
```
user A ──▶ kthread X ──▶ user A ──▶ kthread Y ──▶ user B
   │           │            │            │           │
   mm=A    mm=NULL       mm=A         mm=NULL      mm=B
active=A active_mm=A   active=A   active_mm=A   active=B
            │                         │              │
            │                         │              └─ настоящий load_cr3
            │                         │                 (наконец сменили процесс)
            └─ ничего не делает  ─────┘
               (всё ещё CR3 = A.pgd)
```
...
┌─────────────┬─────────────┬─────────────┬─────────────────────┐
│ PREEMPT     │ SOFTIRQ     │ HARDIRQ     │ NMI / NEED_RESCHED  │
│ (8 бит)     │ (8 бит)     │ (4 бита)    │ + флаги             │
└─────────────┴─────────────┴─────────────┴─────────────────────┘
```
...
```
       ...
       ▼                    ▼                    ▼
┌──────────────┐    ┌───────────────┐    ┌────────────────┐
│stop_sched_   │    │ dl_sched_     │    │ rt_sched_      │  ... и т.д. вниз
│class         │ ─▶ │ class         │ ─▶ │ class          │
│              │    │ (DEADLINE)    │    │ (FIFO/RR)      │
└──────────────┘    └───────────────┘    └────────────────┘
       │                                          │
       │ migration threads,                       │ ─▶ fair_sched_class (CFS/EEVDF)
       │ stop_machine                             │ ─▶ idle_sched_class
       │ (наивысший приор.)                       │
```
...
- [setjmp/longjmp и ucontext](../threads/userspace_context_switching.md) — userspace-аналог: переключение контекста без
...
- [Реализация потоков (clone)](../threads/thread_implementation_clone.md) — флаги `CLONE_*`, что именно shared между
