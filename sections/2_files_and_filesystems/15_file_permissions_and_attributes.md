...
Это разные операции! Вы можете разрешить читать код программы (r), но не позволить её выполнять (x).
...
| **x** | Выполнить файл          | **Входить в директорию** (cd), искать в ней файлы |
...
int lchmod(const char *pathname, mode_t mode); // не POSIX
```
...
chmod("file.txt", S_IRUSR | S_IWUSR | S_IRGRP | S_IROTH);  // 0644
```
...
- С SUID битом программа выполняется с правами **владельца файла**
  -典型例: `/usr/bin/passwd` принадлежит root и имеет SUID, поэтому обычный пользователь может менять свой пароль
...
| **T**  | top of directory      | Зарезервировано для использования ядром                                                 |
