...
Освобождает файловый дескриптор и связанные с ним ресурсы (уменьшает счётчик ссылок на "открытое файловое описание").
...
- область между старым концом файла и позицией записи превратится в **«дыру»** (hole) — диапазон логических нулей,
  который на современных файловых системах (ext4, xfs и т.д.) **физически не занимает места на диске** (такой файл
  называется **sparse file**). (не проверенный факт)
