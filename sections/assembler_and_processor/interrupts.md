...
```
 ...
 │   CS        (сегм. кода)     │
 │   RIP       (адрес возврата) │
 │   Error code (если есть)     │  ← только у #PF, #GP, #SS и др.
 └──────────────────────────────┘
        │
...
Структура одного gate descriptor (16 байт, Interrupt Gate):
┌────────────┬───────────┬─────┬─────┬──────────────┐
│ offset[15:0]│ seg select│ IST │type │offset[31:16]│  байты 0..7
├────────────┴───────────┴─────┴─────┴──────────────┤
│              offset[63:32]                        │  байты 8..15
└───────────────────────────────────────────────────┘
...
- [Параллелизм на уровне инструкций](instruction_level_parallelism.md) — спекулятивное исполнение и его взаимодействие с прерываниями
- [Стековые фреймы и вызовы функций](stack_frames_and_function_calls.md) — red zone и почему ядро компилируется с `-mno-red-zone`
