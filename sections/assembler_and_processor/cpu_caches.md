...
Граф переходов (`local` — операция своего ядра, `remote` — snoop-сообщение от чужого ядра):

```mermaid
stateDiagram-v2
    I --> E: local read miss<br/>(линии нет ни у кого)
    I --> S: local read miss<br/>(линия есть у других)
    I --> M: local write<br/>(read-for-ownership)
    E --> M: local write
    E --> S: remote read (snoop hit)
    E --> I: remote write (snoop invalidate)
    S --> M: local write<br/>(invalidate во все L1)
    S --> I: remote write (snoop invalidate)
    M --> S: remote read<br/>(write-back в RAM)
    M --> I: remote write<br/>(write-back, затем invalidate)

    note left of E
        E и M — эксклюзивны:
        линия только в этом кэше
    end note
    note right of S
        S — линия в нескольких кэшах,
        запись требует invalidate
    end note
```
