...
## Правила выбора memory_order

Практический алгоритм: сначала ответь, **зачем** этой операции синхронизация, и поставь **минимальный достаточный**
порядок. Почти все случаи закрываются деревом решений.

```mermaid
flowchart TD
    P{Публикуешь данные<br/>через этот атомик?} -->|да, запись| REL[release]
    P -->|нет| C{Потребляешь опубликованное:<br/>будешь читать данные,<br/>охраняемые атомиком?}
    C -->|да, чтение| ACQ[acquire]
    C -->|нет| M{И публикуешь,<br/>и потребляешь<br/>в одном RMW?}
    M -->|да| AR[acq_rel]
    M -->|нет| S{Корректность зависит<br/>от единого порядка<br/>над ≥2 атомиками?}
    S -->|да| SC[seq_cst]
    S -->|нет| RLX[relaxed]
```
...
- **Публикуешь данные → `release` на записи.** Сначала обычными записями заполняешь данные, затем release-операцией
  делаешь их достижимыми (ставишь флаг/указатель). release выталкивает все предыдущие записи «перед» этой публикацией.
- **Потребляешь опубликованное → `acquire` на чтении.** Прочитал атомик и собираешься читать данные, на которые он
  «указывает» (поля узла, переменную под флагом) — нужен acquire, иначе данные могут быть ещё не видны.
- **И то, и другое в одном RMW → `acq_rel`.** Операция read-modify-write, которая одновременно забирает чужие записи и
  публикует своё решение: `fetch_sub` на refcount (видит все прошлые `release`, последний decrement публикует «я
  последний»), разблокировка спинлока, helping в lock-free.
- **Атомик ничего не охраняет → `relaxed`.** Счётчик статистики, флаг без сопутствующих данных, указатель, который ты
  используешь только как **значение** (не разыменуешь) — синхронизация не нужна.
- **Нужен единый глобальный порядок над ≥2 атомиками → `seq_cst`.** release/acquire упорядочивают **пару** «продюсер→
  консьюмер» на одной ячейке, но **не** дают общего согласованного порядка между разными ячейками. Классика — store
  buffering / Dekker: два потока пишут **разные** флаги и читают **чужой**; release/acquire тут не спасают (StoreLoad-
  переупорядочивание остаётся), нужен seq_cst или `mfence` (см. раздел «Store buffering» ниже).

Тонкости:

- **release и acquire ходят парой.** release без парного acquire (и наоборот) не создаёт happens-before — это «мост,
  построенный с одного берега». Всегда проверяй, что acquire-чтение действительно прочитало именно тот release-store.
- **CAS: success и failure задаются отдельно.** *success* — по правилам выше (release если публикуешь, acquire если
  потом разыменуешь результат, acq_rel если и то и другое). *failure* — это просто перечитанное значение: `relaxed`,
  если используешь его как значение; `acquire`, если будешь его **разыменовывать**. Ровно отсюда асимметрия Treiber
  stack: в `push` провал = `relaxed` (указатель лишь кладём в `next`), в `pop` провал = `acquire` (указатель сразу
  разыменуем) — разбор в [lock-free структурах](lock_free_structures.md).
- **Прагматика по умолчанию.** Сомневаешься — начни с `seq_cst` (он и так дефолт у `std::atomic`), добейся
  корректности, и ослабляй **точечно**, обосновывая каждое ослабление happens-before-цепочкой. На x86 acquire-load и
  release-store почти бесплатны (обычные `mov`), а seq_cst-store стоит `mfence`; на ARM разница ещё заметнее. Ручной
  relaxed-тюнинг оправдан лишь в горячих lock-free структурах, где он измеримо помогает. Совет Hans Boehm для
  прикладного кода: «пишите мьютексы, не умничайте».
